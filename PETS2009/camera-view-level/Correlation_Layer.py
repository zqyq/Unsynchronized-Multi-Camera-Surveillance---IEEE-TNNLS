...
class Correlation_Layer(Layer):
    ...
    def call(self, x):
        ...
        feature_A_flatten = tf.reshape(feature_A, [b, h*w, c])
        feature_A_flatten = tf.transpose(feature_A_flatten, [0, 2, 1])

        feature_B_flatten = tf.reshape(feature_B, [b, h*w, c])

        corr_AB = tf.matmul(feature_B_flatten, feature_A_flatten)
        corr_AB = tf.reshape(corr_AB, [b, h, w, h*w])
