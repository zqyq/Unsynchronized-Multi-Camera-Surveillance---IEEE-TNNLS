...
def datagen_v3(h5file_view1, h5file_view2, h5file_view3, h5file_GP,
    ...
    density_maps2_u = np.zeros([1, dmp_h, dmp_w, 1])
    images2 = np.zeros([1, img_h, img_w, 1])
    images2_u = np.zeros([1, img_h, img_w, 1])
    for i in h5file_view2:
        h5file_view2_i = i
        with h5py.File(h5file_view2_i, 'r') as f:
            images_i_0 = f['images'].value       # [2*f_offset:, :, :, :] # [:-2*f_offset, :, :, :]
            density_maps_i_0 = f['density_maps'].value         #[2*f_offset:, :, :, :] # [:-2*f_offset, :, :, :]

            images_i = images_i_0[np.abs(f_offset):-np.abs(f_offset), :, :, :]
            density_maps_i = density_maps_i_0[np.abs(f_offset):-np.abs(f_offset), :, :, :]

            num_view2 = images_i.shape[0]
            #num_view2 = images_i[f_offset:-f_offset, :, :, :].shape[0]

            ## set ramdom index
            random_index_view2 = np.random.randint(-f_offset, f_offset, num_view2)
            random_index_view2 = random_index_view2 + np.asarray(range(f_offset, num_view2 + f_offset))

            density_maps_i_u = density_maps_i_0[random_index_view2, :, :, :]
            images_i_u = images_i_0[random_index_view2, :, :, :]

        density_maps2 = np.concatenate([density_maps2, density_maps_i], 0)
        images2 = np.concatenate([images2, images_i], 0)

        density_maps2_u = np.concatenate([density_maps2_u, density_maps_i_u], 0)
        images2_u = np.concatenate([images2_u, images_i_u], 0)

    density_maps2 = density_maps2[1:, :, :, :]
    images2 = images2[1:, :, :, :]

    density_maps2_u = density_maps2_u[1:, :, :, :]
    images2_u = images2_u[1:, :, :, :]

    print(images2.shape[0])
    print(images2_u.shape[0])
    ...
    density_maps3_u = np.zeros([1, dmp_h, dmp_w, 1])

    images3 = np.zeros([1, img_h, img_w, 1])
    images3_u = np.zeros([1, img_h, img_w, 1])
    for i in h5file_view3:
        ...
        with h5py.File(h5file_view3_i, 'r') as f:
            ...
            num_view3 = images_i.shape[0]

            ## set ramdom index
            random_index_view3 = np.random.randint(-f_offset, f_offset, num_view3)
            random_index_view3 = random_index_view3 + np.asarray(range(f_offset, num_view3 + f_offset))

            density_maps_i_u = density_maps_i_0[random_index_view3, :, :, :]
            images_i_u = images_i_0[random_index_view3, :, :, :]

        density_maps3 = np.concatenate([density_maps3, density_maps_i], 0)
        images3 = np.concatenate([images3, images_i], 0)

        density_maps3_u = np.concatenate([density_maps3_u, density_maps_i_u], 0)
        images3_u = np.concatenate([images3_u, images_i_u], 0)

    density_maps3 = density_maps3[1:, :, :, :]
    images3 = images3[1:, :, :, :]
    density_maps3_u = density_maps3_u[1:, :, :, :]
    images3_u = images3_u[1:, :, :, :]

    # print(images3.shape[0])
    # print(images3_u.shape[0])


    ##### Ground-plane density maps  ######
    ...
    while 1:
        ...
        if nb_patch_used + batch_size < patches_per_set + 1: #or (ind_image == 0 and nb_patch_used == 0):
            ...
            dmap_s0 = dmap
            dmap_s1 = conv_process_batch(dmap_s0, pad=0, stride=2, filter_size=2)
            dmap_s2 = conv_process_batch(dmap_s1, pad=0, stride=2, filter_size=2)

            dmap2 = conv_process_batch(dmap2, pad=0, stride=4, filter_size=4)
            dmap2_s0 = dmap2
            dmap2_s1 = conv_process_batch(dmap2_s0, pad=0, stride=2, filter_size=2)
            dmap2_s2 = conv_process_batch(dmap2_s1, pad=0, stride=2, filter_size=2)

            dmap3 = conv_process_batch(dmap3, pad=0, stride=4, filter_size=4)
            dmap3_s0 = dmap3
            dmap3_s1 = conv_process_batch(dmap3_s0, pad=0, stride=2, filter_size=2)
            dmap3_s2 = conv_process_batch(dmap3_s1, pad=0, stride=2, filter_size=2)

            dmap4 = conv_process_batch(dmap4, pad=0, stride=4, filter_size=4) * 1000


            # Y_list.append(dmap_s0)
            # Y_list.append(dmap_s1)
            # Y_list.append(dmap_s2)
            #
            # Y_list.append(dmap2_s0)
            # Y_list.append(dmap2_s1)
            # Y_list.append(dmap2_s2)
            #
            # Y_list.append(dmap3_s0)
            # Y_list.append(dmap3_s1)
            # Y_list.append(dmap3_s2)

            Y_list.append(dmap4)
            Y_list.append(dmap4)

            # add the virtual loss:
            Y_list.append(dmap_s0[:, :, :, 0]*0)
            Y_list.append(dmap_s0[:, :, :, 0]*0)
            Y_list.append(dmap_s0[:, :, :, 0]*0)
